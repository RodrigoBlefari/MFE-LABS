...
function createOutlet({ mountPoint = document.body, append = false } = {}) {
...
async function mount(props = {}) {
  ...
  const {
    ...
    tagline = "single-spa-angular + lifecycle bootstrap/mount/unmount para conviv\xEAncia com Native Federation.",
    log = true,
    variant = "full",
    metrics = {}
  } = props;
  ...
  const updatePerformance = (data = {}) => {
    ...
    metricsMap.count.textContent = typeof data.count === "number" && Number.isFinite(data.count) && data.count > 0 ? String(data.count) : metricsMap.count.textContent || "0";
  };
  ...
  details.innerHTML = `
    <div class="ssa-detail-group">
      ...
      <ul class="ssa-detail-list">
        <li><strong>Lifecycle:</strong> bootstrap/mount/unmount padr\xE3o Single-SPA</li>
        <li><strong>Stack:</strong> Angular 15 + single-spa-angular (adapter ESM)</li>
        <li><strong>Vers\xE3o alvo:</strong> Single-SPA 5.x</li>
      </ul>
    </div>
    <div class="ssa-detail-group">
      <h4>Exemplo pratico</h4>
      <p>Aplica\xE7\xE3o Angular 15 coexistindo com React e Vue em um orquestrador Single-SPA corporativo.</p>
    </div>
    <div class="ssa-detail-group">
      <h4>Como consumir</h4>
      <ul class="ssa-detail-list">
...
async function unmount(props = {}) {
  ...
  const target = host instanceof Element ? host : props && props.outlet instanceof Element ? props.outlet : null;
